...
data_macro_sex <- VDMT_diet[ ,c("ID", "SEX", "KCAL", "PROT", "TFAT", "CARB", "VITD")]
data_macro_sex
...
colnames(data_macro_sex)

VitD_supp <- full_join(data_macro_sex, supplements, by="ID")
colnames(VitD_supp)

vitd_supplements <- VitD_supp[ , c('Cohort', 'VITD')]
ggplot(vitd_supplements, aes(x = VITD, fill = as.factor(Cohort))) +
  geom_histogram(color = 7,
                 alpha = 0.3,
                 position = 'identity') +
  labs(title = "Dietary Vitamin D Intake per Supplement Group",
       x = "Vitamin D (micrograms)",
       y = "Number of ASA records",
       fill = "Supplement")
